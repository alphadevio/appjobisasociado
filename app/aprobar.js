...
const Aprobar = () => {
    ...
    navigation.setOptions({
      ...
      headerRight: () => (
        ...
        <View style={{marginTop:6, marginLeft:10}}>
          <Text style={{color:activeColors.text, fontWeight:'bold', fontSize:20, marginTop:3}}>Aprobar Servicio</Text>
        </View> 
  ...
  const getUser = async () => {
    try{
      ...
      console.log('CLIENTE:',response.result)
  ...
  const aprobarServicio = async () => {
    const body = {
      id: solicitud,
      estatus: 2
    }
    try {
      let response = await api.aprobarSolicitud(body); 
      let decriptado = await api.decrypt(response.message, "private")
      console.log('DECRIPTADO',decriptado)
      juanson = JSON.parse(decriptado)
      if(juanson.status === 'OK'){
        let toast = Toast.show('Servicio aprobado con éxito.', {
  ...
  return(
    <View style={[styles.container, {backgroundColor:activeColors.bg}]}>
      ...
      <View style={styles.desc}>
        ...
        <Pressable style={styles.solicitar} onPress={aprobarServicio}>
          <Button_Grad texto={'Aprobar'} padding={10} width={'100%'}/>
        </Pressable>
      </View>
    </View>
  </View>
